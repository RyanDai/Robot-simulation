...
public class Tester {
    ...
    /** The maximum change in gripper length in one step */
    ...
    /** Minimum gripper segment length */
    public static final double MIN_GRIPPER_LENGTH = 0.03;
    /** Maximum gripper segment length */
	...
	public boolean isValidStep(ArmConfig cfg0, ArmConfig cfg1) {
		if (cfg0.getJointCount() != cfg1.getJointCount()) {
            ...
            return false;
        } else if (cfg0.maxGripperDiff(cfg1) > MAX_GRIPPER_STEP + maxError) {
            return false;
		} else if (cfg0.getBaseCenter().distance(cfg1.getBaseCenter()) > MAX_BASE_STEP + maxError) {
			return false;
		}
		return true;
	}
     ...
     * Checks that joint angles are within the allowable range
     */
    public boolean testGripperLengths(int testNo, boolean verbose) {
        System.out.println(String.format("Test #%d: Gripper length limits", testNo));
        List<Integer> badStates = getInvalidGripperLengthStates();
        if (!badStates.isEmpty()) {
            System.out.println(String.format(
                    "FAILED: Invalid gripper length for %d of %d state(s).",
                    badStates.size(), ps.getPath().size()));
            if (verbose) {
                if (verbose) {
                    System.out.println("Line for each invalid cfg:");
    ...
    /**
     * Returns the path indices of any states with invalid gripper lengths.
     *
     * @return the path indices of any states with invalid joint angles.
     */
    public List<Integer> getInvalidGripperLengthStates() {
        List<Integer> badStates = new ArrayList<Integer>();
        List<ArmConfig> path = ps.getPath();
        for (int i = 0; i < path.size(); i++) {
            if (!hasValidGripperLengths(path.get(i))) {
                badStates.add(i);
            }
        }
        return badStates;
    ...
    /**
     * Checks if all gripper lengths are within the limits
     *
     * @param cfg
     * 			The configuration to test
     * @return true if all gripper lengths are within the limits
     */
	public boolean hasValidGripperLengths(ArmConfig cfg) {
        ...
        for (Double length : gripperLengths) {
            if (length <= MIN_GRIPPER_LENGTH - maxError) {
                return false;
            } else if (length >= MAX_GRIPPER_LENGTH + maxError) {
                return false;
            }
        }
	...
	public boolean testByName(String testName, int testNo, boolean verbose) {
		switch (testName.toLowerCase()) {
            ...
            case "gripper":
                return testGripperLengths(testNo, verbose);
		    case "self-collision":
